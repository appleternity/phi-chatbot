"""
Integration tests for OpenRouter embedding provider.

Tests OpenRouter API integration with mocked responses to verify:
- API authentication (valid/invalid keys)
- Retry logic on HTTP 5xx errors
...
class TestOpenRouterAuthentication:
    """T050: Test OpenRouter API authentication with mocked responses."""

    def test_valid_api_key_authentication(self):
        """T050: Valid API key should initialize provider successfully."""
        # Valid API key initialization should succeed
        provider = OpenRouterEmbeddingProvider(api_key="sk-or-valid-key-123")
    ...
    def test_invalid_api_key_raises_error(self):
        """T050: Invalid/empty API key should raise ValueError."""
    ...
    def test_authentication_failure_on_api_call(self, mock_openai_class):
        ...
        with pytest.raises(RuntimeError, match="OpenRouter embedding generation failed"):
...
class TestOpenRouterRetryLogic:
    """T051: Test OpenRouter retry logic on HTTP 5xx errors."""

    @patch("app.embeddings.openrouter_provider.openai.OpenAI")
    def test_retry_on_server_error_with_exponential_backoff(self, mock_openai_class):
        """T051: HTTP 5xx errors should trigger retry with exponential backoff."""
        ...
        mock_client.embeddings.create.side_effect = [
            ...
            api_error,  # Second attempt: 500 error
            mock_response,  # Third attempt: success
        ]
        mock_openai_class.return_value = mock_client

        provider = OpenRouterEmbeddingProvider(api_key="sk-or-valid-key")
    ...
    @patch("app.embeddings.openrouter_provider.openai.OpenAI")
    def test_retry_exhaustion_raises_runtime_error(self, mock_openai_class):
        """T051: Exhausted retries should raise RuntimeError with original error."""
        # Setup mock to always fail with 503
        mock_client = MagicMock()
        api_error = APIError(message="Service unavailable", request=Mock(), body=None)
        ...
        with patch("time.sleep"):  # Mock sleep to speed up test
            with pytest.raises(RuntimeError, match="OpenRouter embedding generation failed"):
                provider.encode("test retry exhaustion")
    ...
    def test_rate_limit_error_triggers_retry(self, mock_openai_class):
        """T051: HTTP 429 rate limit should trigger retry (special 4xx case)."""
...
class TestOpenRouterBatchProcessing:
    """T052: Test OpenRouter batch processing with automatic splitting."""

    @patch("app.embeddings.openrouter_provider.openai.OpenAI")
    def test_single_batch_under_limit(self, mock_openai_class):
        """T052: Texts under batch_size should be processed in single API call."""
        ...
        provider = OpenRouterEmbeddingProvider(api_key="sk-or-valid-key", batch_size=100)
        ...
        assert call_args.kwargs["model"] == "qwen/qwen3-embedding-0.6b"
    ...
    def test_automatic_batch_splitting(self, mock_openai_class):
        """T052: Texts exceeding batch_size should be automatically split into multiple batches."""
        ...
        # Verify batch sizes in each call
    ...
    def test_exact_batch_size_boundary(self, mock_openai_class):
        """T052: Exactly batch_size texts should be processed in single batch."""
    ...
    @patch("app.embeddings.openrouter_provider.openai.OpenAI")
    def test_custom_batch_size(self, mock_openai_class):
        """T052: Custom batch_size should be respected during splitting."""
        # Setup mock responses for custom batch size
        mock_client = MagicMock()
        ...
        # Mock will be called 3 times: 50, 50, 20 texts
        mock_client.embeddings.create.side_effect = [
            create_batch_response(50),  # Batch 1
            create_batch_response(50),  # Batch 2
            create_batch_response(20),  # Batch 3
        ]
        mock_openai_class.return_value = mock_client

        # Use custom batch_size=50
        provider = OpenRouterEmbeddingProvider(api_key="sk-or-valid-key", batch_size=50)

        # Process 120 texts (should split into 3 batches: 50, 50, 20)
        ...
        # Verify batch sizes
        calls = mock_client.embeddings.create.call_args_list
        assert len(calls[0].kwargs["input"]) == 50
        assert len(calls[1].kwargs["input"]) == 50
        assert len(calls[2].kwargs["input"]) == 20
