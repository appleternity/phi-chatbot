"""
Chunk extraction component for chunking system.

This module implements Phase 2 of the chunking pipeline: extracting chunks
from documents based on semantic structure with metadata and contextual prefixes.
"""
...
TEXT_EXTRACTION_PROMPT = """You are extracting a specific section of text from a document for chunking purposes.
...
5. Output ONLY the extracted text, no explanations or preamble
...
class ChunkExtractor:
    """Chunk extractor for Phase 2 of chunking pipeline"""
    ...
    def generate_llm_response_key(
        ...
        Args:
            ...
            operation: Operation type (e.g., "extract_text", "generate_metadata")
            section_info: Optional section-specific info (title + summary)
    ...
    def extract_chunks(
        ...
        """
        Extract chunks from document based on semantic structure.
        ...
        Behavior:
            1. For each section in structure:
               a. Use LLM to extract full text for section (based on title + summary)
        ...
        for i, section in enumerate(structure.sections):
            try:
                # Step 1: Extract text for this section using LLM
    ...
    def _extract_section_text(
        ...
        """
        Extract text for a specific section using LLM.
        ...
        section_info = f"{section_title}_{section_summary}"
        cache_key = None
        if self.cache_store:
            cache_key = self.generate_llm_response_key(
                document.content, self.model, "extract_text", section_info
            )
